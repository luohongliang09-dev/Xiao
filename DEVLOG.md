...
- **做了什么**：
  - 后端新增 `POST /api/ingest/batch`（多文件，按扩展名自动分流：txt/md/docx → 文字解析，pdf/图片 → 视觉描述）
  - 前端「导入资料」页加「导入整个文件夹」按钮（隐藏 `<input webkitdirectory>` 选文件夹），批量发送、按成功/失败数提示
  - `client.ts` 加 `ingestBatch()`
- **验证**：3 个文件一次批量上传全部入库成功
- **阻塞项**：无
